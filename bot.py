...
def load_memo():
    try:
        ...
        sh = gc.open_by_url(spreadsheet_url).worksheet(sheet_name)
        memo_dict = {}
        for row in sh.get_all_values():
            if row[0] in memo_dict:
                memo_dict[row[0]].append(row[1])
            else:
                memo_dict[row[0]] = [row[1]]
        print("Loaded memo from Google Spreadsheet")
        return memo_dict
    except Exception as e:
        print(f"Error while loading memo from Google Spreadsheet: {e}")
        return {}
        print(f"Error while loading memo from Google Spreadsheet: {e}")
        memo_dict = {}
    ...
    try:
        ...
        for memo_topic, memo_contents in memo_dict.items():
            for i, memo_content in enumerate(memo_contents):
                rows.append([memo_topic, memo_content])
        sh.insert_rows(rows)
        print("Saved memo to Google Spreadsheet")
    except Exception as e:
...
async def memo(ctx, *, memo):
    ...
    sh.append_row([user, memo])

    # Send a confirmation message
    await ctx.send(f"{user}, 메모가 정상적으로 저장됐습니")
